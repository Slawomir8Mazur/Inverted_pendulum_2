...
class DynamicLearning:
        ...
        for i in range(num_episodes):
            ...
            for j in range(5):
                ...
                for a in range(0, 2):
                    ...
                    target.append(self.give_reward(next_record.record[feats], **pass_to_reward)[['fi']].sum(axis=1)[0])
                ...
                print("Epochs: %d\tFrom:\t %0.2f \tTo:\t %0.2f" % (epochs,
                                                                   self.give_reward(record.record[feats], **pass_to_reward)[['fi']].sum(axis=1)[0],
                                                                   self.give_reward(record.last_movement.iloc[[0], :][feats], **pass_to_reward)[['fi']].sum(axis=1)[0]))
    ...
    def teach(self, num_episodes=100, range_of_angle=(1, 360), epochs=1, pass_to_reward={}):
        ...
        for i in range(num_episodes):
            ...
            for j in range(10):
                ...
                record.move(force_table=[(self.ft_table.loc[a, 'forces'],
                                          self.ft_table.loc[a, 'times'])])
                target = (self.give_reward(record.record[feats], **pass_to_reward)[['fi']].sum(axis=1)[0]
    ...
    def check(self, angle=20, num_episodes=20):
        ...
        r.visualize(['fi'], r.stack_of_movement, separately=True)
...
d.model.load_weights('new_reinforced_learning_greedy_60x36_lin_i5_epoch1.h5')
d.check(num_episodes=500)
plt.show()


"""d = DynamicLearning(eps=0.1)
d.model.save_weights('new_reinforced_learning_greedy_60x36_lin_0.h5')
d.check(num_episodes=80)
for i, epoch in enumerate([20, 15, 10, 5, 3, 1]):
    d.teach_greedy(num_episodes=1000, epochs=epoch, pass_to_reward={"x_mul": 0}, range_of_angle=(-50, 50))
    d.model.save_weights('new_reinforced_learning_greedy_60x36_lin_i%d_epoch%d.h5' % (i, epoch))
    d.check(num_episodes=50)
plt.show()"""
